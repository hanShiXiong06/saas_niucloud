            ...
            <div class="mt-[10px]">
                <el-table :data="tableData.data" size="large" v-loading="tableData.loading">
                    ...
                    <el-table-column :label="t('sumCount')" min-width="160">
                        <template #default="{ row }">
                            <span v-if="row.receive_type == 1 && row.sum_count != '-1'">{{ row.remain_count || 0 }} / {{
                    row.sum_count || 0 }}</span>
                    ...
                    <el-table-column :label="t('receiveTypeTime')" min-width="210">
                        <template #default="{ row }">
                            <div v-if="row.receive_type == 1">
                                <div v-if="parseInt(row.start_time) != 0 && parseInt(row.end_time) != 0"
                                    class="flex flex-col">
                    ...
                    <el-table-column :label="t('operation')" fixed="right" align="right" min-width="160">
                        <template #default="{ row }">
                            ...
                            <el-button type="primary" link @click="editEvent(row)"
                                v-if="row.status == 0 || row.status == 1">{{ t('edit') }}</el-button>
                            <el-button type="primary" link @click="deleteEvent(row)" v-if="row.status != 1">{{
                    t('delete') }}</el-button>
                            <el-button type="primary" link @click="closeEvent(row)" v-if="row.status == 1">{{ t('close')
                                }}</el-button>
...
import { getCouponList, deleteCoupon, closeCoupon, getCouponStatusList } from '@/addon/tk_jhkd/api/coupon'
import { ElMessageBox, FormInstance } from 'element-plus'
import { t } from '@/lang'
import couponSpreadPopup from '@/addon/tk_jhkd/views/coupon/components/coupon-spread-popup.vue'
import { setTablePageStorage, getTablePageStorage } from "@/utils/common";
import couponCollection from '@/addon/tk_jhkd/views/coupon/components/coupon-collection.vue'
...
// 添加优惠券
const handleChange = () => {
    router.push('/tk_jhkd/coupon/add')
}
// 编辑优惠券
const editEvent = (data: any) => {
    router.push('/tk_jhkd/coupon/edit?id=' + data.id)
}
