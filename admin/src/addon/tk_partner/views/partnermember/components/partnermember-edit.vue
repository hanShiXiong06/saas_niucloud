<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updatePartnerMember') : t('addPartnerMember')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('isPartner')" >
                    <el-input v-model="formData.is_partner" clearable :placeholder="t('isPartnerPlaceholder')" class="input-width" />
                </el-form-item>
...
import { addPartnerMember, editPartnerMember, getPartnerMemberInfo, getWithMemberList } from '@/addon/tk_partner/api/partnermember'
...
,
    is_partner: [
        { required: true, message: t('isPartnerPlaceholder'), trigger: 'blur' },
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editPartnerMember : addPartnerMember
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getPartnerMemberInfo(row.id)).data
