<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updatePartnerOrder') : t('addPartnerOrder')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addPartnerOrder, editPartnerOrder, getPartnerOrderInfo, getWithMemberList, getWithLevelList } from '@/addon/tk_partner/api/partnerorder'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editPartnerOrder : addPartnerOrder
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getPartnerOrderInfo(row.id)).data
