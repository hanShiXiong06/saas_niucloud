<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateCommissionOrder') : t('addCommissionOrder')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('profit')" prop="profit">
                    <el-input v-model="formData.profit" clearable :placeholder="t('profitPlaceholder')" class="input-width" />
                </el-form-item>
...
import { addCommissionOrder, editCommissionOrder, getCommissionOrderInfo, getWithMemberList } from '@/addon/tk_partner/api/commissionorder'
...
,
    profit: [
        { required: true, message: t('profitPlaceholder'), trigger: 'blur' },
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editCommissionOrder : addCommissionOrder
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getCommissionOrderInfo(row.id)).data
