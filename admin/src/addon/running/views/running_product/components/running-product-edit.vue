<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRunningProduct') : t('addRunningProduct')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('sales')" prop="sales">
                    <el-input v-model="formData.sales" clearable :placeholder="t('salesPlaceholder')" class="input-width" />
                </el-form-item>
...
import { addRunningProduct, editRunningProduct, getRunningProductInfo, getWithRunningSchoolList } from '@/addon/running/api/running_product'
...
const formRules = computed(() => {
    return {
    school_id: [
        { required: true, message: t('schoolIdPlaceholder'), trigger: 'blur' },
...
,
    sales: [
        { required: true, message: t('salesPlaceholder'), trigger: 'blur' },
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRunningProduct : addRunningProduct
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRunningProductInfo(row.id)).data
