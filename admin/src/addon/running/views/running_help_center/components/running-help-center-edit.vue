<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRunningHelpCenter') : t('addRunningHelpCenter')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addRunningHelpCenter, editRunningHelpCenter, getRunningHelpCenterInfo } from '@/addon/running/api/running_help_center'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRunningHelpCenter : addRunningHelpCenter
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRunningHelpCenterInfo(row.id)).data
