<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRunningOrderLog') : t('addRunningOrderLog')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('operatorId')" prop="operator_id">
                    <el-select class="input-width" v-model="formData.operator_id" clearable :placeholder="t('operatorIdPlaceholder')">
                       <el-option label="请选择" value=""></el-option>
                        <el-option
...
import { addRunningOrderLog, editRunningOrderLog, getRunningOrderLogInfo, getWithMemberList } from '@/addon/running/api/running_order_log'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRunningOrderLog : addRunningOrderLog
...
// 获取字典数据
    ...
    const operatorIdList = ref([] as any[])
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRunningOrderLogInfo(row.id)).data
