<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRunningOrderProduct') : t('addRunningOrderProduct')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addRunningOrderProduct, editRunningOrderProduct, getRunningOrderProductInfo } from '@/addon/running/api/running_order_product'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRunningOrderProduct : addRunningOrderProduct
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRunningOrderProductInfo(row.id)).data
