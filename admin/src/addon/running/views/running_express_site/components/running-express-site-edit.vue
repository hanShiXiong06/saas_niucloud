<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRunningExpressSite') : t('addRunningExpressSite')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addRunningExpressSite, editRunningExpressSite, getRunningExpressSiteInfo, getWithRunningSchoolList } from '@/addon/running/api/running_express_site'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRunningExpressSite : addRunningExpressSite
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRunningExpressSiteInfo(row.id)).data
