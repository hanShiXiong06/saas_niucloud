    ...
    <el-form :model="formData" label-width="150px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
      <el-card class="box-card !border-none" shadow="never">
        ...
        <el-form-item :label="t('reserveEarly')">
          <el-input class="input-width" @keyup="filterNumber($event)" v-model="formData.advance" maxlength="5">
            <template #append>{{ t('hour') }}</template>
          </el-input>
        </el-form-item>

        <el-form-item :label="t('reserveNumber')">
          <div>
            <el-input class="input-width" @keyup="filterNumber($event)" v-model="formData.max" maxlength="5">
              <template #append>{{ t('personNum') }}</template>
            </el-input>
            <p class="text-[12px] text-[#a9a9a9]">{{ t('reserveNumberTips') }}</p>
          </div>
        </el-form-item>
      </el-card>
    </el-form>
...
import { t } from '@/lang'
import { setReserveConfig, getReserveConfig } from '@/addon/vipcard/api/vipcard'
