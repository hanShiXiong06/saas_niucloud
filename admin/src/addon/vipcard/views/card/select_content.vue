<template>
    <div>
        <el-form :inline="true" :model="tableData.searchParam" ref="searchFormRef">
            <el-form-item :label="t('cardGoodsName')" prop="goods_name">
                <el-input v-model="tableData.searchParam.goods_name" :placeholder="t('cardGoodsNamePlaceholder')" />
            </el-form-item>
        ...
        <el-table :data="tableData.data" size="large" ref="tableRef" v-loading="tableData.loading">
            ...
            <el-table-column :label="t('cardGoodsInfo')" :show-overflow-tooltip="true" min-width="20%" align="left">
                <template #default="{ row }">
            ...
            <el-table-column prop="card_type_name" :label="t('cardType')" min-width="20%">
                <template #default="{ row }">
            ...
            <el-table-column prop="sale_num" :label="t('cardSelectContentSaleNum')" min-width="20%" />
        </el-table>
...
import { t } from '@/lang'
import { getCardSelectList} from '@/addon/vipcard/api/vipcard'
...
const getData = () => {
    if (selectData.goods_id == 0) {
        ElMessage({
            type: 'warning',
            message: `${ t('cardSelectContentTips') }`
        });
        return;
    }

    return {
        name: 'VIPCARD_CARD_DETAIL',
        title: selectData.goods_name,
        url: `/addon/vipcard/pages/card/detail?id=${ selectData.goods_id }`,
        action: '',
