				...
				<view v-show="diyStore.editComponent.layout == 'horizontal'">
					...
					<el-form-item :label="t('graphicNavShowStyle')">
						<el-radio-group v-model="diyStore.editComponent.showStyle" @change="changeShowStyle">
							...
							<el-radio :label="'singleSlide'">{{ diyStore.editComponent.pageCount == 2 ?
								t('graphicNavStyleMultiLine') : t('graphicNavStyleSingleSlide') }}</el-radio>
			...
			<el-form label-width="90px" class="px-[10px]">
				...
				<div ref="imageBoxRef">
					<div v-for="(item, index) in diyStore.editComponent.list" :key="item.id"
						...
						<el-form-item label="类型类111型">
							<el-radio-group v-model="item.type">
								<el-radio :label="'system'">系统链接</el-radio>
								<el-radio :label="'act'">活动链接</el-radio>
							</el-radio-group>
						</el-form-item>
						<el-form-item v-if="item.type === 'system'" label="系统链接">
							<diy-link v-model="item.link" />
						</el-form-item>
						<el-form-item v-if="item.type === 'act'" label="活动链接">
							<act-link v-model="item.act" />
						</el-form-item>
...
import actLink from "@/addon/tk_cps/views/components/act-link.vue";
