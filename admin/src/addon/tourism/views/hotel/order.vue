<template>
  <div class="main-container">
        ...
        <el-table :data="orderTable.data" size="large" v-loading="orderTable.loading">
            ...
            <template #default="{ row }">
              <div class="flex items-center cursor-pointer ">
                <div class="min-w-[60px] h-[60px] flex items-center justify-center mr-[10px]">
                  <img class="max-w-[60px] max-h-[60px]" v-if="row.image_thumb_small" :src="img(row.image_thumb_small)"
                       alt="">
          ...
          <el-table-column :label="t('orderMoney')" min-width="180" align="center">
            <template #default="{ row }">
              {{ row.order_money }}
              <div v-if="row.pay">
                {{ row.member_id !== row.pay.main_id && row.pay.status == 2 ? row.pay.pay_type_name : '' }}
              </div>
            </template>
          </el-table-column>
    ...
    <export-sure ref="exportSureDialog" :show="flag" type="tourism_hotel_order" :searchParam="orderTable.searchParam" @close="handleClose" />
