<template>
    <div class="main-container">
        <el-card class="card !border-none mb-[15px]" shadow="never">
            <el-page-header :content="pageName" :icon="ArrowLeft" @click="router.push({ path: '/tourism/order/scenic' })"/>
        </el-card>
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" class="page-form" v-loading="loading">
            <el-card class="box-card !border-none relative" shadow="never" v-if="formData">
                ...
                <el-row class="row-bg" >
                    ...
                    <el-col :span="6" v-if="formData.pay">
                        <el-form-item :label="t('payTypeName')">
                            <div class="input-width">{{ formData.pay.type_name }} </div>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6" v-if="formData.pay && formData.member_id !== formData.pay.main_id && formData.pay.status == 2">
                        <el-form-item>
                ...
                <el-row class="row-bg" v-for="(item, index) in formData.buyer_info" :key="index">
                    <el-col :span="6">
                    ...
                    <el-col :span="6">
                        <el-form-item :label="t('touristCardNo')">
                            <div class="input-width">{{ item.id_card }}</div>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-table :data="formData.item" size="large">
                    ...
                    <el-table-column :label="t('ticketInfo')" min-width="200" align="left">
                        <template #default="{ row }">
                            <div class="flex">
                                <img class="w-[100px] max-h-[60px] mr-[10px]" v-if="row.goods_image" :src="img(row.goods_image)" />
                                <div>
                                    <p>{{ formData.scenic.scenic_name }}</p>
                                    <p class="text-sm text-gray-400">{{ row.goods_name }}</p>
                                </div>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="goods_money" :label="t('ticketMoney')" min-width="50" align="left" />
                    <el-table-column prop="num" :label="t('ticketNum')" min-width="50" align="center" />
                </el-table>
                ...
                <div style="height: 300px">
                    <div class="flex" v-for="(items, index) in formData.order_log" :key="index">
                        <div class="mr-[20px] min-w-[71px]">
                            <div class="leading-[1] text-[14px] w-[100px] flex justify-end">{{ items.action_time.split(' ')[0] }}</div>
                            <div class="leading-[1] text-[14px] w-[100px] flex justify-end mt-[5px]">{{ items.action_time.split(' ')[1] }}</div>
                        </div>
                        <div>
...
import { ref } from 'vue'
import { t } from '@/lang'
import { getScenicOrderInfo } from '@/addon/tourism/api/tourism'
...
const setFormData = async (orderId: number = 0) => {
    loading.value = true
    formData.value = null
    await getScenicOrderInfo(orderId)
        .then(({ data }) => {
            formData.value = data
        })
        .catch(() => {
