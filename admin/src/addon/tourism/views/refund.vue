...
import { t } from '@/lang'
import { getRefundList, getRefundStatus, confirmRefund, refuseRefund } from '@/addon/tourism/api/tourism'
...
// 获取维权状态
const orderStatus = ref<AnyObject[]>([])
...
const handleConfirm = (info: AnyObject) => {
    ElMessageBox.confirm(
        ...
        confirmRefund({ refund_id: info.refund_id }).then(() => {
                loadRefundList()
        }).catch()
    }).catch(() => {
