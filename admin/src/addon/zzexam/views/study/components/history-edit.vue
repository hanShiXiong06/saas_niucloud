<template>
    <el-dialog v-model="showDialog" :title="formData.history_id ? t('updateHistory') : t('addHistory')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addHistory, editHistory, getHistoryInfo } from '@/addon/zzexam/api/study'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.history_id ? editHistory : addHistory
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getHistoryInfo(row.history_id)).data
