<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateCollect') : t('addCollect')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addCollect, editCollect, getCollectInfo } from '@/addon/zzexam/api/collect'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editCollect : addCollect
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getCollectInfo(row.id)).data
