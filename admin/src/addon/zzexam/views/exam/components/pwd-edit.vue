<template>
    <el-dialog v-model="showDialog" :title="formData.sign_id ? t('updatePwd') : t('addPwd')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addPwd, editPwd, getPwdInfo } from '@/addon/zzexam/api/exam'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.sign_id ? editPwd : addPwd
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getPwdInfo(row.sign_id)).data
