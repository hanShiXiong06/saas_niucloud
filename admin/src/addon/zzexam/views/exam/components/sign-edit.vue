<template>
    <el-dialog v-model="showDialog" :title="formData.sign_id ? t('updateSign') : t('addSign')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addSign, editSign, getSignInfo } from '@/addon/zzexam/api/exam'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.sign_id ? editSign : addSign
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getSignInfo(row.sign_id)).data
