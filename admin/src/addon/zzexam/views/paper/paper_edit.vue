        ...
        <el-card class="box-card !border-none" shadow="never">
            <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form">
                <el-form-item :label="t('categoryId')" prop="category_id">
                    <el-cascader v-model="formData.category_id" :options="paperCategoryOptions" :props="paperCategoryProps" clearable filterable popper-class="choice" :placeholder="t('categoryIdPlaceholder')" />
                    <div class="ml-[10px]">
                        <span class="cursor-pointer text-primary mr-[10px]" @click="refreshPaperCategoryFn(true)">{{ t('refresh') }}</span>
                ...
                <el-form-item :label="t('mode')" prop="mode" v-if="formData.paper_id">
				...
				<el-form-item :label="t('抽题分类')" prop="question_category_ids" v-if="formData.mode == 1">
					<div class="w-[100%] flex">
						<el-cascader v-model="formData.question_category_ids" :options="questionCategoryOptions" :props="questionCategoryProps" clearable filterable popper-class="choice" :placeholder="t('全部分类')" />
                ...
                <el-form-item :label="t('题目难度')" v-if="formData.mode == 1">
					<div class="w-[100%]">
						<el-checkbox-group v-model="formData.difficultys">
						    <el-checkbox :label="item.value" v-for="(item,index) in difficultyList" :key="index">
								{{ item.name }}
						    </el-checkbox>
						</el-checkbox-group>
					</div>
                            ...
                            <tbody class="options-new">
                                <template v-if="formData.question_rules.length">
                                    <tr v-for="(item,index) in formData.question_rules" :key="index">
                                        <td>{{item.name}}</td>
                                        <td>
                                            <el-input-number :min="0" v-model="item.num" />
                                        </td>
                                        <td>
                    						<el-input-number type="number" :min="0" v-model="item.score" />
                                        </td>
										<td>
											<el-switch
											  v-model="item.isterm"
                    					...
                    					<td>
                    						<el-input-number type="number" :min="1" v-model="item.sort" />
                    					</td>
                                    </tr>
                                </template>
                ...
                <el-form-item :label="t('passScore')" prop="pass_score">
                    <el-input-number v-model="formData.pass_score" clearable :placeholder="t('passScorePlaceholder')" class="input-width" :min="0" />
                </el-form-item>
...
const onSave = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
       if (valid) {
           ...
           const save = id ? editPaper : addPaper
           save(data).then(res => {
               loading.value = false
               history.back()
           }).catch(err => {
