<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updatePaperQuestion') : t('addPaperQuestion')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addPaperQuestion, editPaperQuestion, getPaperQuestionInfo } from '@/addon/zzexam/api/paper'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editPaperQuestion : addPaperQuestion
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getPaperQuestionInfo(row.id)).data
