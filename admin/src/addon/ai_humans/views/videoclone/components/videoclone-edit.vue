<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateVideoClone') : t('addVideoClone')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('callbackUrl')" >
                    <el-input v-model="formData.callback_url" clearable :placeholder="t('callbackUrlPlaceholder')" class="input-width" />
                </el-form-item>
...
import { addVideoClone, editVideoClone, getVideoCloneInfo, getWithMemberList } from '@/addon/ai_humans/api/videoclone'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editVideoClone : addVideoClone
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getVideoCloneInfo(row.id)).data
