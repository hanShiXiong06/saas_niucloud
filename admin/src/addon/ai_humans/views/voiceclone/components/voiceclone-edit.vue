<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateVoiceClone') : t('addVoiceClone')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addVoiceClone, editVoiceClone, getVoiceCloneInfo, getWithMemberList } from '@/addon/ai_humans/api/voiceclone'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editVoiceClone : addVoiceClone
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getVoiceCloneInfo(row.id)).data
