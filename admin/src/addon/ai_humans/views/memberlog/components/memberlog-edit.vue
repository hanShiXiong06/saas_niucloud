<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateMemberLog') : t('addMemberLog')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
        <el-form :model="formData" label-width="120px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-form-item :label="t('changType')" >
                    <el-input v-model="formData.chang_type" clearable :placeholder="t('changTypePlaceholder')" class="input-width" />
                </el-form-item>
...
import { addMemberLog, editMemberLog, getMemberLogInfo, getWithMemberList } from '@/addon/ai_humans/api/memberlog'
...
,
    chang_type: [
        { required: true, message: t('changTypePlaceholder'), trigger: 'blur' },
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editMemberLog : addMemberLog
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getMemberLogInfo(row.id)).data
