<template>
    <view class="bg-[var(--page-bg-color)] min-h-screen p-2" :style="themeColor()">
        <mescroll-body ref="mescrollRef" top="64rpx" @init="mescrollInit" @down="downCallback" @up="getVoiceListFn">
            <view v-if="list.length > 0" class="">
                <view v-for="(item, index) in list" :key="index">
                    <view class="flex justify-between items-center p-3 rounded-[8rpx] shadow-sm mb-2"
                        :style="{ background: 'linear-gradient(to right, var(--primary-color-light), var(--primary-color-light))' }">
        ...
        <u-popup :show="showPopup" safeAreaInsetBottom="true" @close="showPopup = false" bgColor="transparent"
            ...
            <view v-if="selectedVoice.voice_url != ''" class="p-4 text-center"
            ...
            <view v-if="selectedVoice.voice_url != ''" class="p-4 text-center"
                style="border-bottom: 1px solid var(--primary-color);" @click="copy(img(selectedVoice.voice_url), '')">
...
import { getVoiceList, deleteVoice } from "@/addon/ai_humans/api/voice";
...
const getVoiceListFn = (mescroll: MescrollInstance) => {
    loading.value = false;
