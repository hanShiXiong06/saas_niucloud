		...
		<view class="p-4 bg-gradient-to-br from-[#454337] to-[#5a5749]">
			<view class="flex justify-between items-center">
				...
				<view
					...
					@click="shareEvent()">
					<u-icon :name="img('addon/ai_humans/fenxiao/tgm.png')" size="16" color="#D5C6A9"></u-icon>
				...
				<block v-if="list" v-for="(item, index) in list" :key="index">
					<view class="tk-card !mt-0">
						...
						<view class="flex items-center justify-between">
							...
							<text v-if="item.first_commission > 0" class="text-[#454337] font-bold">
								佣金: {{ item.first_commission }}
							</text>
							<text v-if="item.two_commission > 0" class="text-[#454337] font-bold">
...
import { img, redirect, handleOnloadParams, copy } from '@/utils/common';
import { getMemberCommission } from '@/app/api/member';
import useMemberStore from '@/stores/member'
import { onLoad, onPageScroll, onReachBottom } from '@dcloudio/uni-app';
import {
	getFenxiaoInfo, getFenxiaoOrder, getOrderData
