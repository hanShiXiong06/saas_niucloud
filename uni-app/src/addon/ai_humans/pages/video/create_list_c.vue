<template>
    <view class="bg-[var(--page-bg-color)] min-h-screen p-2" :style="themeColor()">
        ...
        <mescroll-body ref="mescrollRef" top="88rpx" @init="mescrollInit" @down="downCallback"
                    ...
                    <view
                        ...
                        <view class="w-full relative" v-if="item.status == 0 || item.status == 1">
                            ...
                            <view
                                ...
                                <view class="ml-2 px-2 py-1 rounded-full bg-[rgba(var(--primary-color-rgb),0.1)]">
                            ...
                            <view class="relative group">
                                ...
                                <view
                                    ...
                                    <view class="flex-1 mr-4">
                                        <text
                                            class="text-[26rpx] font-medium text-[var(--primary-color)] truncate block tk-sltext w-[120rpx]">{{
                                                item.name }}</text>
                                    ...
                                    <view @click="selectedVideoEvent(item)"
                                        class="ml-2 py-1 px-3 rounded-full relative overflow-hidden">
                        ...
                        <view class="w-full" v-if="item.status == 3">
                            ...
                            <!-- 底部信息栏 -->
                            <view
                                class="px-2 py-3 flex items-center justify-between border-t border-[rgba(var(--primary-color-rgb),0.1)]">
                                <view class="text-[24rpx] text-[var(--primary-color)] truncate flex-1 font-medium">
                                    {{ item.name }}
                                </view>
                                <view @click="selectedVideoEvent(item)"
                                    class="ml-2 py-1 rounded-full relative overflow-hidden"
...
// 布局主题 - 修复themeColor undefined问题
const layoutTheme = computed(() => {
    // 如果有主题方法则返回，否则返回空对象
    return (window as any).themeColor || (() => ({}));
});
