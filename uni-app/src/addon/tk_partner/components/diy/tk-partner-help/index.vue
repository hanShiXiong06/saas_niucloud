				...
				<view class="rounded-lg shadow-md overflow-hidden flex items-center">
					...
					<view class="flex-1 p-3 pl-0">
						<view class="font-bold"
							:style="{ fontSize: diyComponent.titlesize + 'px', color: diyComponent.titlecolor }">默认标题
						</view>
		...
		<scroll-view v-else scroll-y @scrolltolower="loadMore" @refresherrefresh="refresh" refresher-enabled
			...
			<view class="">
				<view v-for="(item, index) in listData" :key="index"
					@click.stop="redirect({ url: `/addon/tk_partner/pages/help/detail?id=${item.id}` })"
...
// @ts-ignore - 忽略模块类型声明错误
import { getHelpList } from '@/addon/tk_partner/api/help';
...
const goLogin = () => {
	...
	// #endif
	// #ifdef H5
	useLogin().setLoginBack({ url: '/addon/tk_partner/pages/index/index' })
	// #endif
}
