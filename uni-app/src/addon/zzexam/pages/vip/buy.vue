<template>
	<view :style="themeColor()" class="bg-[var(--page-bg-color)] min-h-[100vh]" v-if="loading">
		...
		<u-tabbar :fixed="true" :placeholder="true" :safeAreaInsetBottom="true" v-if="vipList.length">
...
<script setup lang="ts">
	...
	import { getVipList, getVipConfig, orderCreate } from '@/addon/zzexam/api/vip';
	...
	const create = () => {
		...
		if(orderId.value != 0 && tradeType.value != ''){
			payRef.value?.open(tradeType.value, orderId.value,`/addon/zzexam/pages/member/index`);
		}else{
			orderCreate(data).then((res) => {
				orderId.value = res.data.trade_id;
				tradeType.value = res.data.trade_type;
				payRef.value?.open(res.data.trade_type, res.data.trade_id,`/addon/zzexam/pages/member/index`);
			}).catch(() => {
