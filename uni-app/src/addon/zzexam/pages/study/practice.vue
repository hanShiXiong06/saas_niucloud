	...
	<view :style="themeColor()" class="bg-[var(--page-bg-color)] min-h-[100vh]">
		<block v-if="!loading">
			<view class="flex p-[30rpx] bg-white">
				<view>
					<view class="flex">
						<text>{{type == 'errors' ? '错题练习' : '模拟练习'}}</text>
					</view>
				</view>
				<view class="ml-auto">
			...
			<up-transition :show="transitionShow" :mode="transitionMode">
				<view class="m-[30rpx] rounded-[20rpx] bg-white overflow-hidden">
					...
					<view v-if="questionList[currentIndex].show_answer" class="mb-[45rpx]">
						<view class="mx-[45rpx]">
							<u-divider class="m-0" text="正确答案" textColor="#000000"></u-divider>
							<view class="text-[#808080]">
								<text v-for="(item,index) in questionList[currentIndex].c_answer" :key="index">{{index<questionList[currentIndex].c_answer.length-1 ? item +'，' : item}} </text>
							</view>
			...
			<view class="border-[0] border-t-[2rpx] border-solid border-[#f5f5f5] w-[100%] px-[60rpx] bg-[#fff] box-border fixed left-0 bottom-0 tab-bar z-1 items-center pt-[16rpx]">
				<view class="flex justify-between">
					...
					<view class="flex flex-col justify-center items-center" @click="showAnswer">
						<view class="zzexam icon-jiaojuan text-[40rpx]"></view>
						<text class="text-[24rpx] mt-[10rpx]">对答案</text>
					</view>
...
<script setup lang="ts">
	...
	import { getPracticeQueston } from '@/addon/zzexam/api/exam';
	import { addNote,editNote } from '@/addon/zzexam/api/study'
	import { useLogin } from '@/hooks/useLogin'
	import useMemberStore from '@/stores/member'
	import { intervalTime } from '@/addon/zzexam/utils/common'
	import answerSheetPopup from '@/addon/zzexam/pages/exam/components/answer-sheet-popup.vue'
	...
	onLoad((option) => {
		...
		getPracticeQuestonFn();
	})
	
	//获取练习题目
	const getPracticeQuestonFn = () => {
		loading.value = true;
		getPracticeQueston({type:type.value,category_id:categoryId.value}).then((res) => {
			loading.value = false;
	...
	//对答案
	const showAnswer = () => {
		questionList.value[currentIndex.value].show_answer = 1;
	}
