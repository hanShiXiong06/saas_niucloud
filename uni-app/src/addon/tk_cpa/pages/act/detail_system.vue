        ...
        <view 
            ...
            <view class="flex-1">
                <u-button type="error" shape="circle" @click="redirect({ url: '/addon/tk_cpa/pages/sid/index' })"
                    class="shadow-lg h-12 text-base font-medium" plain>开码记录</u-button>
            </view>
            <view class="flex-1 ml-4">
                <u-button type="error" shape="circle" @click="addEvent"
                    class="shadow-lg h-12 text-base font-semibold">新增推广码</u-button>
            </view>
        </view>
        <u-popup :show="showPopup" @close="showPopup = false" mode="center" round="10" :closeable="true">
            <view class="w-[600rpx] p-6">
                <view class="text-lg font-semibold text-gray-900 mb-6 text-center">新增推广码</view>
                ...
                <view class="mt-8">
                    <u-button type="error" shape="circle" @click="submitForm"
                        class="shadow-lg h-12 text-base font-semibold w-full">确认申请</u-button>
                </view>
            </view>
        </u-popup>
    </view>
...
const submitForm = async () => {
    ...
    try {
        ...
        if (res.code == 1) {
            ...
            formData.phone = '';
            setTimeout(() => {
                redirect({ url: '/addon/tk_cpa/pages/sid/detail_system?id=' + res.data.id })
            }, 500);
        } else {
            uni.showToast({
                title: res.msg || '申请失败',
