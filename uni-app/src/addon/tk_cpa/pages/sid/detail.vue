            ...
            <view v-if="sidInfo.promotionUrl != ''"
                ...
                <view class="bg-white p-3">
                    <view class="flex items-center justify-between mb-2">
                        ...
                        <view class="flex flex-wrap items-center gap-2">
                            <button @click="showUrlDialog(sidInfo.promotionUrl)"
                                class="flex items-center justify-center px-2 py-1 rounded-full bg-blue-50 text-blue-500 text-xs font-medium shadow-sm transform transition hover:scale-105 active:scale-95">
                                <text class="text-blue-500 text-xs">查看</text>
                            </button>
                            <button @click="openUrl(sidInfo.promotionUrl)"
                                class="flex items-center justify-center px-2 py-1 rounded-full bg-orange-50 text-orange-500 text-xs font-medium shadow-sm transform transition hover:scale-105 active:scale-95">
                                <text class="text-orange-500 text-xs">打开</text>
                            </button>
                            <button @click="copy(sidInfo.promotionUrl, '已复制做单链接')"
                                class="flex items-center justify-center px-2 py-1 rounded-full bg-gradient-to-r from-green-500 to-green-600 text-white text-xs font-medium shadow-md transform transition hover:scale-105 active:scale-95">
                                <text class="text-white text-xs">复制</text>
                            </button>
                        </view>
                    </view>
                ...
                <view class="bg-white p-3 mt-3">
                    <view class="flex items-center justify-between mb-2">
                        ...
                        <view class="flex flex-wrap items-center gap-2">
                            <button @click="showUrlDialog(sidInfo.queryDataUrl)"
                                class="flex items-center justify-center px-2 py-1 rounded-full bg-blue-50 text-blue-500 text-xs font-medium shadow-sm transform transition hover:scale-105 active:scale-95">
                                <text class="text-blue-500 text-xs">查看</text>
                            </button>
                            <button @click="openUrl(sidInfo.queryDataUrl)"
                                class="flex items-center justify-center px-2 py-1 rounded-full bg-orange-50 text-orange-500 text-xs font-medium shadow-sm transform transition hover:scale-105 active:scale-95">
                                <text class="text-orange-500 text-xs">打开</text>
                            </button>
        ...
        <view class="px-4 pb-28">
            <view v-if="currentTab === 0">
                <view class="bg-white rounded-xl shadow-sm p-4 mb-3">
                    ...
                    <view class="text-gray-600 text-sm leading-relaxed">
                        <u-parse :content="sidInfo.act_info.settlementDesc"
                ...
                <view class="bg-white rounded-xl shadow-lg p-4">
                    ...
                    <view class="text-gray-600 text-sm leading-relaxed">
                        <u-parse :content="sidInfo.act_info.projectDesc"
                            :tagStyle="{ img: 'vertical-align: top;', p: 'overflow: hidden;word-break:break-word;' }"></u-parse>
                    </view>
                </view>
            </view>
            <view v-else>
                <view class="bg-white rounded-xl shadow-sm p-4">
                    ...
                    <view class="text-gray-600 text-sm leading-relaxed">
                        <u-parse :content="sidInfo.act_info.promotionDesc"
                            :tagStyle="{ img: 'vertical-align: top;', p: 'overflow: hidden;word-break:break-word;' }"></u-parse>
                    </view>
                </view>
            </view>
...
const openUrl = (url: string) => {
    ...
    uni.setClipboardData({
        ...
        success: () => {
            uni.showModal({
                title: '提示',
                content: '小程序内无法直接打开链接，链接已复制，请在浏览器中粘贴访问',
                showCancel: false
            });
        }
    });
