	...
	<view class="detail-box" v-if="detailData">
		<view class="tk-card">
          ...
          " :src="detailData.logo" mode="aspectFill"></image>
			...
			<view class="flex mt-3 items-center">
				<view class="">
					<u-tag class="" :text="`按实付` + detailData.plan.ratio + `%返`" size="mini"></u-tag>
				</view>

				<view class="ml-2 mr-2">
					<u-tag :text="`最高可返` + detailData.plan.commission" plain plainFill size="mini"></u-tag>
				</view>

				<view class="">
					<u-tag text="需要用餐评价" v-if="detailData.plan.planType == 1" plain plainFill size="mini"></u-tag>
					<u-tag text="无需评价" v-else plain plainFill size="mini"></u-tag>
				</view>
			</view>
			...
			<view v-if="detailData.sku_list && detailData.sku_list.length > 0" class="flex flex-wrap justify-start">
				<block v-for="(item, index) in detailData.sku_list" :key="index">

					<view class="w-[20%]  p-1 rounded-lg mr-1" v-if="index <= 3">

						<image :src="item.picUrl" mode="aspectFill" style="
            width: 140rpx;
            height: 140rpx;
            background-color: #eeeeee;
            border-radius: 8px;
          "></image>
						...
						<view class="text-xs tk-sltext ">{{ item.name }}</view>
						<view class="flex align-items-end">
							<view class="price">{{ item.actPrice }}</view>
							<view class="oriPrice-line text-xs ml-1">￥{{ item.oriPrice }}</view>
						</view>
					</view>
				</block>
			</view>
			<view class="flex mt-1">
				<u-tag @click="goShop(detailData)" text="查看店铺" size="mini"></u-tag>
			</view>
		</view>
		<view class="tk-card">
			<view class="flex">
			...
			<view class="flex justify-between text-xs mb-2">
				...
				<view class="flex items-center">
					...
					<u-tag v-if="phone" @click="editPhone()" class="ml-2" :text="`修改`" type="error" plain plainFill
						size="mini"></u-tag>
					<u-tag v-if="!phone" @click="editPhone()" class="ml-2" :text="`填写手机号`"
						color="var(--primary-color-light)" bgColor="var(--primary-color)"
						borderColor="var(--primary-color)" plain plainFill size="mini"></u-tag>
				</view>
			</view>
			<view class="line-box w-[100%] mb-3"></view>
			<view class="flex text-xs mb-2 justify-between">
				...
				<view class="flex">
					...
					<view class="ml-2 text-[var(--primary-color)]">{{
						detailData.plan.planTypeDescCh
					}}</view>
				</view>
			</view>
			<view class="line-box mb-3 bg-[var(--primary-help-color)]" style="background-color: var(--primary-color)">
			</view>
		...
		<view class="b-tabbar safe-area-inset-bottom">
			<view class="b-tabbar-back fb items-center p-2">
				...
				<u-button :loading="loading" loadingText="正在报名" color="var(--primary-color)" shape="circle"
					:customStyle="{
						lineHeight: '76rpx',
						margin: '0rpx',
						color: '#fff',
	...
	<u-popup :show="showEditPhone" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4 items-center">
			...
			<view class="flex justify-between mt-2 mb-2">
				<u-button color="#828282" shape="circle" size="big" :customStyle="{
					lineHeight: '96rpx',
					margin: '0rpx',
					color: '#fff',
					width: '200rpx',
				...
				<u-button :loading="loading" loadingText="正在报名" color="var(--primary-color)" shape="circle" size="big"
					:customStyle="{
						lineHeight: '96rpx',
						margin: '0rpx',
						color: '#fff',
	...
	<!-- <u-popup :show="showSubmitOrder" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4">
			...
			<view class="flex justify-end mt-2 mb-2">
				...
				<u-button color="var(--primary-color)" shape="circle" size="small"
...
import { timeChange, getLocationData } from "@/addon/tk_cps/utils/ts/common";
...
const submitOrder = async () => {
	...
	locationData.value = getLocationData();
	loading.value = true;
	try {
