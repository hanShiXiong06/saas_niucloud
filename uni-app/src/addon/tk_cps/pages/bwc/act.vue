<template>
	<view class="detail-box">
		<view class="flex items-center mb-[16rpx] mt-[32rpx] flex-right justify-between mr-2">
			...
			<view class="flex items-center" @click="locationVal.reposition()">
				<view>
					<u-icon name="map" color="var(--primary-help-color)" size="18"></u-icon>
				</view>
				<view class="text-[var(--primary-help-color)] text-xs tk-sltext">
					{{ !systemStore.diyAddressInfo ? "选择位置" : systemStore.diyAddressInfo.community }}
				</view>
				<view>
					<u-icon name="arrow-right" color="var(--primary-help-color)" size="18"></u-icon>
				</view>
			</view>
		</view>
		<view class="">
        ...
        " :src="item.logo" mode="aspectFill"></image>
			...
			<view class="" v-for="(item1, index1) in item.planList" :key="index1">
				...
				<view @click="goDetail(item1)" class="flex justify-between items-center">
					<view class="flex">
						<view class="">
							<u-tag :text="`最高返` + item1.commission" bgColor="var(--primary-color)"
								borderColor="var(--primary-color)" size="mini"></u-tag>
						</view>
						<view class="ml-2">
							...
							<u-tag text="无需评价" v-else type="error" plain plainFill size="mini"
								bgColor="var(--primary-color-light2)" borderColor="var(--primary-color-light2)"
								color="var(--primary-color)"></u-tag>
						</view>
					</view>
					<view class="flex items-center">
						<view class="kucun">
							...
							<u-line-progress :percentage="(item1.restStock / item1.totalStock) * 100"
								activeColor="var(--primary-color)" height="5" :showText="false"></u-line-progress>
						</view>

						<u-icon name="arrow-right" color="#cccccc" size="10"></u-icon>
						<view class="ml-2">
							<u-tag v-if="item1.restStock > 0" @click="goDetail(item1)" :text="`去报名`"
								bgColor="var(--primary-color)" borderColor="var(--primary-color)"></u-tag>
	...
	<up-loading-icon class="mt-4 mb-4" :show="loading" mode="circle" inactive-color="var(--primary-color)"
