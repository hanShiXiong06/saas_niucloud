				...
				<view class="flex flex-col ml-2 w-[100%]" style="">
					...
					<view class="flex mt-2 items-center flex-wrap">
						...
						<view class="mt-1">
							...
							<u-tag :text="`按实付` + orderInfo.commissionRatio + `%返`" bgColor="var(--primary-color)"
						...
						<view class="ml-2 mr-2 mt-1">
							...
							<u-tag :text="`最高可返` + orderInfo.maxAmount" plain plainFill size="mini"></u-tag>

						</view>

						<view class="mt-1">
							<u-tag text="需要用餐评价" v-if="orderInfo.planType == 1" type="primary" plain plainFill
								size="mini"></u-tag>
			...
			<view class="card1 p-2">
				...
				<view class="mt-4">
					...
					<view class="flex justify-center">
						<u-button color="var(--primary-color)" shape="circle"
				...
				<view class="mt-4 mb-4">
					...
					<view v-if="orderInfo.planType == 1" class="text-[var(--primary-color)]">
...
const hongbao = (e) => {
	...
	if (e.platform == 1) {
		...
		diyRedirect({ url: '/addon/tk_cps/pages/index?type=1&act_id=150&style=embedded', name: 'hongbao' })

	}

	if (e.platform == 2) {

		diyRedirect({ url: '/addon/tk_cps/pages/index?type=1&act_id=89&style=embedded', name: 'hongbao' })
