<template>
	<view :style="warpCss">
		...
		<view class="diy-graphic-nav relative">
			<view v-if="diyComponent.layout == 'vertical'" class="graphic-nav">
				<view class="graphic-nav-item" v-for="(item, index) in diyComponent.list" :key="item.id">
					...
					<view @click="diyStore.toRedirect(item.type == 'system' ? item.link : item.act)"
						:class="['flex items-center justify-between py-3 px-4', index == 0 ? 'border-t-0' : 'border-t']">
			...
			<view class="pt-[10rpx]"
				v-else-if="diyComponent.layout == 'horizontal' && diyComponent.showStyle == 'pageSlide'">
				<swiper class="graphic-nav swiper relative"
					:style="{ height: swiperHeight, width: '95%', margin: '0 auto', opacity: swiperHeight ? 1 : 0 }"
					...
					<swiper-item class="graphic-nav-wrap flex flex-wrap"
						...
						<template v-for="(item, index) in diyComponent.list">
							...
							<view :class="[diyComponent.mode]" :key="item.id" v-if="swiperCondition(index, numItem)"
								:style="{ width: 100 / diyComponent.rowCount + '%' }">

								<view @click="diyStore.toRedirect(item.type == 'system' ? item.link : item.act)"
									class="graphic-nav-item flex flex-col items-center py-2">
				...
				<view class="graphic-nav-indicator-dot" v-if="shouldShowIndicator && swiperHeight">
					<div class="dots-wrap" :class="[diyComponent.swiper.indicatorAlign]">
						<div v-for="(item, index) in Math.ceil(diyComponent.list.length / (diyComponent.pageCount * diyComponent.rowCount))"
							...
							:style="{ background: index == swiperIndex ? diyComponent.swiper.indicatorActiveColor : diyComponent.swiper.indicatorColor }">
						</div>
					</div>
				</view>
			</view>
			...
			<view :style="{ width: '98%', margin: '0 auto' }"
				...
				<view class="uni-scroll-view-content">
					<!-- #endif -->
					<scroll-view class="graphic-nav-wrap whitespace-nowrap"
						:scroll-x="diyComponent.showStyle == 'singleSlide'"
						v-for="(numItem, numIndex) in getSlideRowNum()">
						<template v-for="(item, index) in diyComponent.list">

							<view v-if="isShowslideTemp(index, numItem)"
								@click="diyStore.toRedirect(item.type == 'system' ? item.link : item.act)"
			...
			<scroll-view v-else :scroll-x="diyComponent.showStyle == 'singleSlide'"
				:class="['graphic-nav', 'graphic-nav-' + diyComponent.showStyle]" class=" py-[10rpx]">
				<!-- #ifdef MP -->
				<view class="uni-scroll-view-content">
					<!-- #endif -->
					...
					<view class="graphic-nav-item" :class="{ 'flex-shrink-0': diyComponent.showStyle == 'singleSlide' }"
						...
						<view @click="diyStore.toRedirect(item.type == 'system' ? item.link : item.act)"
							class="flex flex-col items-center box-border py-2">
							<view class="graphic-img relative flex items-center justify-center w-10 h-10"
								v-if="diyComponent.mode != 'text'"
								:style="{ width: diyComponent.imageSize * 2 + 'rpx', height: diyComponent.imageSize * 2 + 'rpx' }">
								<image v-if="item.imageUrl" :src="img(item.imageUrl)" mode="aspectFill"
