<template>
	<x-skeleton :type="skeleton.type" :loading="skeleton.loading" :config="skeleton.config"
		v-if="couponList && Object.keys(couponList).length > 0">
		<view :style="warpCss" class="overflow-hidden">
			...
			<view v-if="diyComponent.style == 'style-1'" class="coupon-wrap style-1 relative">
				<scroll-view scroll-x="true" class="coupon-list"
					:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/style1_bg2.png') + ')', 'background-size': '100%', 'background-repeat': 'no-repeat' }">
					<view class="coupon-class">
						<block v-if="couponList.length > 1">
							<view v-for="(item, index) in couponList" :key="index"
								...
								:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/coupon_item_bg.png') + ')', 'background-size': '100%', 'background-repeat': 'no-repeat' }"
						...
						<block v-else>
							<view v-for="(item, index) in couponList" :key="index"
								...
								:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/style1_bg4.png') + ')', 'background-size': '100%', 'background-repeat': 'no-repeat' }"
								@click="couponItemLink(item)">
								<view class="flex price-font text-[var(--price-text-color)] items-baseline">
									<text class="text-[36rpx] mt-[16rpx] mr-[4rpx]">￥</text>
									<text class="text-[85rpx] font-500 max-w-[170rpx] truncate">{{
										parseFloat(item.price) }}</text>
								</view>
								<view
									class="border-0 border-dashed border-r-[2rpx] border-[#FF323C] absolute left-[40%] top-[46rpx] bottom-0 w-[2rpx]">
								</view>
								<view class="w-[270rpx]">
									<view class="flex items-center mt-[auto]">
										<text
											class="rounded-[4rpx] bg-[#fff3f0] text-[#f2333c] border-[2rpx] border-solid border-[#f2333c] text-[22rpx] px-[6rpx] pb-[4rpx] pt-[6rpx] flex items-center justify-center whitespace-nowrap">{{
												item.type_name }}</text>
										<text class="ml-[4rpx] text-[#f2333c] max-w-[184rpx] truncate">{{ item.title
										}}</text>
									</view>
									<view class="text-[#f2333c] text-[30rpx] font-500 mt-[10rpx] w-[270rpx] truncate">
				...
				<view
					...
					:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/style1_bg.png') + ')', 'background-size': '100% 130rpx', 'background-repeat': 'no-repeat' }">
					<view class="flex flex-col">
						<text class="text-[30rpx] text-[#fff] font-400">{{ diyComponent.couponTitle }}</text>
						<text class="text-[20rpx] text-[rgba(255,255,255,.8)] mt-[10rpx]">{{ diyComponent.couponSubTitle
						}}</text>
					</view>
					<text v-if="diyComponent.btnText" @click="toLink('/addon/tk_jhkd/pages/coupon/list')"
			...
			<view v-else-if="diyComponent.style == 'style-2'" class="coupon-wrap style-2 relative">
				...
				<view
					...
					:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/style2_bg.png') + ')', 'background-size': '290rpx 170rpx', 'background-repeat': 'no-repeat' }">
					<text class="text-[30rpx] text-[#fff] font-500">{{ diyComponent.couponTitle }}</text>
					<text class="text-[20rpx] text-[rgba(255,255,255,.8)] mt-[14rpx]">{{ diyComponent.couponSubTitle
					}}</text>
					<text v-if="diyComponent.btnText" @click="toLink('/addon/tk_jhkd/pages/coupon/list')"
						class="bg-[#fff] text-[#FF4142] text-[22rpx] min-w-[100rpx] px-[24rpx] box-border h-[50rpx] leading-[50rpx] text-center coupon-buy-btn mt-auto">{{
							diyComponent.btnText }}</text>
				</view>
			</view>

			<view v-else-if="diyComponent.style == 'style-3'" class="coupon-wrap style-3 relative"
				:style="{ 'background-image': 'url(' + img('addon/tk_jhkd/diy/coupon/style3_bg.jpg') + ')', 'background-size': '100% 204rpx', 'background-repeat': 'no-repeat' }">
				<view class="desc flex flex-col">
					<text class="text-[30rpx] text-[#fff] font-500">{{ diyComponent.couponTitle }}</text>
					<text class="text-[22rpx] text-[rgba(255,255,255,.8)] mt-[10rpx]">{{ diyComponent.couponSubTitle
					}}</text>
					<text v-if="diyComponent.btnText" @click="toLink('/addon/tk_jhkd/pages/coupon/list')"
				...
				<scroll-view scroll-x="true" class="coupon-list" v-if="couponList.length > 1">
					<view v-for="(item, index) in couponList" :key="index"
						...
						<view class="coupon-item-content">
							...
							<view
								class="mt-[auto] flex items-baseline justify-center w-full truncate price-font text-[#fff]">
								<text class="text-[24rpx]">￥</text>
								<text class="text-[44rpx] font-bold truncate">{{ parseFloat(item.price) }}</text>
							</view>
						</view>
						<view class="text-[#303133] text-[22rpx] truncate max-w-[120rpx] mt-[12rpx]">
							{{ item.min_condition_money == '0.00' ? '无门槛' :
								('满' + parseFloat(item.min_condition_money) + '元可用') }}</view>
					</view>
				</scroll-view>
				<view v-if="couponList.length == 1"
					...
					<view class="flex items-center coupon-item-content">
						...
						<view class="flex flex-col">
							<view class="text-[#fff] text-[28rpx] mb-[14rpx]">{{ couponList[0].min_condition_money ==
								'0.00' ? '无门槛' : ('满' + parseFloat(couponList[0].min_condition_money) + '元可用') }}</view>
							<view class="flex items-center">
								<text
									class="bg-[#fff] mr-[10rpx] text-[red] text-[20rpx] px-[10rpx] py-[8rpx] rounded-[20rpx]">{{
										couponList[0].type_name }}</text>
			...
			<view v-else-if="diyComponent.style == 'style-4'" class="coupon-wrap style-4 relative"
				:style="couponStyle4Css">
				<view class="desc flex items-center pt-[6rpx] pb-[26rpx]"
					@click="toLink('/addon/tk_jhkd/pages/coupon/list')">
					<text class="text-[32rpx] text-[#fff] font-500" :style="{ color: diyComponent.titleColor }">{{
						diyComponent.couponTitle }}</text>
					<text class="text-[22rpx] text-[rgba(255,255,255,.8)] ml-[10rpx]"
						:style="{ color: diyComponent.subTitleColor }">{{ diyComponent.couponSubTitle }}</text>
				</view>
				<scroll-view scroll-x="true" class="coupon-list">
					<view v-for="(item, index) in couponList" :key="index"
						...
						<view class="flex flex-col">
							<view class="text-[28rpx] font-500" :style="{ 'color': diyComponent.couponItem.textColor }">
								{{ item.type_name }}</view>
							<view class="text-[#666] text-[24rpx] truncate max-w-[180rpx] mt-[12rpx]"
								:style="{ 'color': diyComponent.couponItem.subTextColor }">{{ item.min_condition_money
									==
									'0.00' ? '无门槛' : ('满' + parseFloat(item.min_condition_money) + '元可用') }}</view>
						</view>
					</view>
