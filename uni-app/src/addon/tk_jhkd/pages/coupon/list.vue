<template>
	<view class="bg-[var(--page-bg-color)] min-h-[100vh] overflow-hidden" :style="themeColor()">
		<view class="coupon-header fixed  left-0 right-0 top-0 z-10080">
			<!-- #ifdef MP-WEIXIN -->
			<view
				:style="{ height: headStyle, backgroundImage: 'url(' + img('addon/tk_jhkd/coupon/coupon_uniapp.png') + ')', backgroundSize: '100%', backgroundPosition: 'bottom', backgroundRepeat: 'no-repeat' }">
				<top-tabbar :data="param" class="top-header" />
			</view>
			<!-- #endif -->
			<!-- #ifdef H5 -->
			<view class="h-[364rpx]"
				:style="{ backgroundImage: 'url(' + img('addon/tk_jhkd/coupon/coupon_uniapp.png') + ')', backgroundSize: 'cover', backgroundPosition: 'bottom', backgroundRepeat: 'no-repeat' }">
			</view>
			<!-- #endif -->
			<view
				...
				<view class="flex items-center" :class="{ 'text-primary font-500': searchType == 'create_time' }"
					@click="searchTypeFn('create_time')">
					<text class="mr-[4rpx]">最新</text>
					<text v-if="create_time == 'asc'"
						class="text-[18rpx] text-[var(--text-color-light6)] nc-iconfont nc-icon-a-xiangshangV6xx1"
						:class="{ '!text-primary': searchType == 'create_time' }"></text>
					<text v-else
						class="text-[18rpx] text-[var(--text-color-light6)]  nc-iconfont nc-icon-a-xiangxiaV6xx1"
						:class="{ '!text-primary': searchType == 'create_time' }"></text>
				</view>
				<view class="flex items-center" :class="{ 'text-primary font-500': searchType == 'price' }"
					@click="searchTypeFn('price')">
					<text class="mr-[4rpx]">价格</text>
					<text v-if="price == 'asc'"
						class="text-[18rpx]  text-[var(--text-color-light6)] nc-iconfont nc-icon-a-xiangshangV6xx1"
						:class="{ '!text-primary': searchType == 'price' }"></text>
					<text v-else
						class="text-[18rpx] text-[var(--text-color-light6)] nc-iconfont nc-icon-a-xiangxiaV6xx1"
						:class="{ '!text-primary': searchType == 'price' }"></text>
				</view>
				<view class="flex items-center" :class="{ 'text-primary font-500': searchType == 'type' }"
					@click="searchTypeFn('type')">
					<view class="w-[2rpx] h-[28rpx] bg-gradient-to-b from-[#333] to-[#fff] mr-[20rpx] flex-shrink-0">
					</view>
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" height="auto" @up="getCouponListFn"
			:top="mescrollTop">
			<view v-if="list.length" class="pb-[var(--top-m)] sidebar-margin">
				<template v-for="(item, index) in list">
					<view v-if="item.btnType === 'collected'"
						...
						:style="{ backgroundImage: 'url(' + img('addon/tk_jhkd/coupon/coupn_loot.png') + ')' }"
						...
						<view
							...
							<view class="flex-1 box-border ml-[10rpx]">
								...
								<view
									...
									<text v-else> 有效期至{{ item.valid_end_time ? item.valid_end_time.slice(0, 10) : ''
									}}</text>
								</view>
							</view>
						</view>
						<view class="pr-[20rpx] pl-[34rpx]">
							<button class="flex-center"
								:style="{ width: '150rpx', height: '60rpx', color: '#fff', fontSize: '24rpx', padding: '0', backgroundColor: 'var(--primary-color-disabled)', border: 'none', opacity: '1', borderRadius: '30rpx' }"
								disabled>已领完</button>
						</view>
						<view
					...
					<view v-else
						class="flex items-center relative w-[100%] rounded-[var(--rounded-big)] overflow-hidden bg-[#fff] py-[20rpx] background-size"
						:class="{ 'mt-[var(--top-m)]': index }" @click="toDetail(item.id)">
						<view
							...
							<view class="flex-1 box-border ml-[10rpx]">
								...
								<view
									...
									<text v-if="item.valid_type == 1">领取之日起<text>{{ item.length || ''
									}}</text>天内有效</text>
									<text v-else> 有效期至<text>{{ item.valid_end_time ? item.valid_end_time.slice(0, 10) :
										'' }}</text></text>
								</view>
							</view>
						</view>
						<view v-if="item.btnType === 'collecting'" @click.stop="collecting(item.id, index)"
							...
							<button class="flex-center"
								:style="{ width: '150rpx', height: '60rpx', color: '#fff', fontSize: '24rpx', padding: '0', backgroundColor: 'var(--primary-color)', border: 'none', borderRadius: '30rpx' }">立即领取</button>
						</view>
						<view v-if="item.btnType === 'using'" @click.stop="toLink(item.id)"
							class="pr-[20rpx] pl-[34rpx]">
							<button class="flex-center"
								:style="{ width: '150rpx', height: '60rpx', color: 'var(--primary-color)', fontSize: '24rpx', padding: '0', backgroundColor: 'transparent', border: '2rpx solid var(--primary-color)', borderRadius: '30rpx' }">去使用</button>
						</view>
						<view
							class="absolute top-0 right-[190rpx]  h-[10rpx] w-[20rpx] rounded-br-[20rpx] rounded-bl-[20rpx] bg-[var(--page-bg-color)] ">
						</view>
			...
			<!-- <view class=""> -->
			<mescroll-empty v-if="!list.length && !loading" :option="{ tip: '暂无优惠券', btnText: '去寄件' }"
				@emptyclick="redirect({ url: '/addon/tk_jhkd/pages/ordersubmit' })"></mescroll-empty>
			<!-- </view> -->
		</mescroll-body>

		<u-popup :show="typePopup" mode="top" @close="typePopup = false" :customStyle="{ top: typePopupTopVal }"
...
import { getCouponList, getCoupon, getMyCouponType } from '@/addon/tk_jhkd/api/coupon'
...
const getCouponListFn = (mescroll: any) => {
	loading.value = true;
	let data: object = {
		page: mescroll.num,
		limit: mescroll.size,
...
const toDetail = (coupon_id: any) => {
	redirect({ url: '/addon/tk_jhkd/pages/coupon/detail', param: { coupon_id } })
}
const toLink = (coupon_id: any) => {
	redirect({ url: '/addon/tk_jhkd/pages/index', param: { coupon_id } })
}
