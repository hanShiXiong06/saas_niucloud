<template>
    <view :style="themeColor()">
        <view v-if="Object.keys(detail).length && !loading" class="overflow-hidden min-h-screen bg-style relative"
            :style="{ 'background': 'url(' + img('addon/tk_jhkd/coupon/coupon_bg.png') + ') no-repeat' }">
            <!-- #ifdef MP -->
            <top-tabbar :data="topTabbarData" :scrollBool="topTabarObj.getScrollBool()" />
            <!-- #endif -->
            <view class="relative mt-[236rpx] w-[100%] h-[932rpx]"
                :style="{ 'background': 'url(' + img('addon/tk_jhkd/coupon/coupon_bg_02.png') + ') center / contain no-repeat' }">
                <view v-if="detail.limit_count"
                    :style="{ 'background': 'url(' + img('addon/tk_jhkd/coupon/top_tab.png') + ') center / cover no-repeat', 'transform': 'translateX(-50%)' }"
                ...
                <view class="flex justify-center items-center mt-[20rpx]">
                    <text v-if="detail.btnType === 'collected'"
                        ...
                        :style="{ 'background': 'url(' + img('addon/tk_jhkd/coupon/coupon_btn_02.png') + ')  center / contain no-repeat' }">已领完
                    </text>
                    <text v-if="detail.btnType === 'collecting'"
                        class="!leading-[100rpx] text-center text-[#E22D17] text-[46rpx] min-w-[240rpx] h-[106rpx]"
                        :style="{ 'background': 'url(' + img('addon/tk_jhkd/coupon/coupon_btn.png') + ') center / contain no-repeat' }"
...
import { getCouponInfo, getCoupon, getCouponQrocde } from '@/addon/tk_jhkd/api/coupon'
...
onLoad((option: any) => {
    ...
    if (!option.coupon_id) {
        let parameter = {
            url: '/addon/tk_jhkd/pages/coupon/list',
            title: '优惠券不存在'
        };
        goback(parameter)
    } else {
        getCouponInfoFn(option.coupon_id)
        getCouponQrocdeFn(option.coupon_id)
    }
})

const getCouponInfoFn = (id: number) => {
    loading.value = true
    getCouponInfo(id).then((res: any) => {
        ...
        detail.value = {}
        setTimeout(() => {
            redirect({ url: '/addon/tk_jhkd/pages/index', mode: 'reLaunch' })
        }, 600)
    })
}

const getCouponQrocdeFn = (id: any) => {
    // #ifdef H5
    ...
    // #ifdef MP-WEIXIN
    getCouponQrocde(id).then((res: any) => {
        if (res.data) {
