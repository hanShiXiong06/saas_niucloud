<template>
	<view :style="warpCss">
		...
		<view class="diy-vipcard-card relative goods-wrap py-[30rpx] px-[32rpx]" v-if="loading && list?.length">
			<view class="flex justify-between items-center w-full mb-4">
				<text class="font-bold">{{t('card')}}</text>
				<view class=" flex items-center" @click="redirect({ url: '/addon/vipcard/pages/card/list', mode: 'reLaunch'})">
				...
				<view :class="['flex',{'mb-3': list.length-1 != index}]" v-for="(item,index) in list" :key="item.way_id" @click="toLink(item)">
					...
					<view class="flex flex-col flex-1 py-[20rpx]">
						...
						<view class="flex items-center mt-auto justify-between">
							<span class="text-xs text-[var(--text-color-light6)] flex-1">{{t('soldOut')}} {{item.sale_num}}</span>
							<button type="primary" class="reserve-btn text-sm flex items-center justify-center mx-0">{{t('cardBtn')}}</button>
						</view>
					</view>
				</view>
			</view>
...
import { t } from '@/locale'
  ...
  const maskLayer = computed(()=>{
	...
	const toLink = (data) => {
		redirect({ url: '/addon/vipcard/pages/card/detail', param: { id: data.goods_id }})
	}

	let list = ref([]);
	let loading = ref(false);
	const getIndexCardListFn = () => {
		let data : object = {
		...
		loading.value = false;
		getIndexCardList(data).then((res) => {
			list.value = res.data;
			loading.value = true;
		}).catch(() => {
			loading.value = true;
  ...
  onMounted(() => {
      ...
      if (diyStore.mode == 'decorate') {
          watch(
              () => diyComponent.value,
              (newValue, oldValue) => {
                  if (newValue && newValue.componentName == 'VipcardCard') {
                      nextTick(() => {
                          const query = uni.createSelectorQuery().in(instance);
                          query.select('.diy-vipcard-card').boundingClientRect((data: any) => {
