<template>
  <view class="content">
    <view class="container">
      ...
      <scroll-view class="scroll-view_H b-t b-b" scroll-x>
        <block v-for="(item, index) in dateArr" :key="index">
          <div
            class="flex-box"
            @click="ispd(item.weekNumber) ? selectDateEvent(index, item) : ''"
            :class="{ borderb: index == dateActive }"
            :style="{
              borderColor: selectedBorderColor,
              background: !ispd(item.weekNumber) ? '#f1f3f7' : '',
              color: !ispd(item.weekNumber) ? '#999' : ''
            }"
          >
            <view class="date-box" :style="{ color: index == dateActive ? selectedTabColor : !ispd(item.weekNumber) ? '#999' : '#333' }">
        ...
        <template v-for="(item, _index) in timeArr" :key="_index">
          <view class="item">
            <view
              ...
              :style="{
                color: isMultiple ? (item.isActive ? selectedItemColor : '#333') : _index == timeActive ? selectedItemColor : '#333',
                borderColor: isMultiple ? item.isActive : _index == timeActive ? selectedBorderColor : ''
              }"
    ...
    <view class="bottom" v-if="isBtnAction">
      <view class="show-time" v-if="!isMultiple && !isSection && !isQuantum"> 预约时间：{{ orderDateTime }} </view>
...
<script>
  ...
  import { getReserveConfig } from '@/addon/vipcard/api/vipcard'
  export default {
    name: 'times',
    model: {
      prop: 'showPop',
    ...
    methods: {
      // 循环判断当前选择项是否有
      ispd(e) {
        const week = this.json.week
        // console.log(e)
        return week.includes(e)
      },
      // 辅助函数：将分钟转换为小时
      ...
      secondsToHM(seconds) {
        const hours = Math.floor(seconds / 3600) // 计算小时 ‌:ml-citation{ref="2,3" data="citationList"}
        const minutes = Math.floor((seconds % 3600) / 60) // 计算剩余分钟 ‌:ml-citation{ref="2,3" data="citationList"}
        // 格式化补零
        return `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:00` // ‌:ml-citation{ref="1,2" data="citationList"}
      },

      initOnload() {
        this.dateArr = initData() // 日期栏初始化
        let timeInterval = this.convertMinutesToHours(this.json.interval)
        let beginTime = this.secondsToHM(this.json.start)
        let endTime = this.secondsToHM(this.json.end)
        let advance = this.json.advance
        if(!this.isInitialized){
            this.dateArr = this.dateArr.map((item, index) =>{
                if (this.ispd(item.weekNumber)) {
                    item.disable = false
                } else {
                    item.disable = true
                }
            ...
            this.dateActive = this.dateArr.findIndex(item => item.disable == false)
            this.isInitialized = true
        }
        
        this.timeArr = initTime(beginTime, endTime, timeInterval, this.isQuantum, advance) //时间选项初始化
        ...
        this.timeArr.forEach((item, index) => {
          ...
          if (this.isQuantum) {
            ...
            if (this.dateActive == 0 && this.selectDate == this.nowDate && currentTime().time > item.time) {
              item.disable = true
            }
