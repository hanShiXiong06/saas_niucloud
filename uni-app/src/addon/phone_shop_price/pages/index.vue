                                    ...
                                    <up-grid-item v-for="(listItem, listIndex) in getFilteredChildList(category)"
                                        ...
                                        <view class="grid-item-content">
                                            ...
                                            <view class="brand-icon"
                                                :class="{ 'vip-locked': listItem.need_vip && !isVip }">
...
const _getPaymentList = async () => {
    ...
    if (res.code === 1) {
        ...
        if (res.data.length === 0) {
            uni.navigateTo({ url: '/addon/phone_shop_price/pages/payment/index' });
        }
    }
};
...
onMounted(async () => {
    ...
    Promise.all([
        ...
        getMemberLevel(),
        _getBannerList()
    ]).then(async ([categoryRes, memberRes]) => {
        if (categoryRes.code === 1 && categoryRes.data) {
            useSubscribeMessage().request("phone_shop_price_order_sign");
