                    ...
                    <view class="flex flex-col justify-between w-full h-[430rpx] mb-[var(--top-m)] rounded-[var(--rounded-big)] overflow-hidden box-border absolute left-0 top-0 z-5">
                        <view class="flex justify-between py-[var(--pad-top-m)] px-[var(--pad-sidebar-m)]">
                            <view class="flex h-[38rpx] px-[10rpx] bg-[rgba(255,255,255,0.9)] rounded-[19rpx]">
                                <text class="mr-[8rpx] iconfont !text-[24rpx] !leading-[38rpx]" :class="{'iconchuzhikaV6mm !text-[#EF000C]':item.giftcard.card_right_type=='balance','iconduihuankaV6mm-1 !text-[#FF7700]':item.giftcard.card_right_type=='goods'}"></text>
                                ...
                                <text class="!text-[22rpx] font-400 !leading-[38rpx]">
                                    <text v-if="item.giftcard.card_right_type=='balance'">{{ t('yuan') }}</text>
                        ...
                        <view class="flex items-center justify-between bg-[rgba(255,255,255,0.9)] h-[80rpx] px-[var(--pad-sidebar-m)] box-border">
                            <block v-if="item.status=='to_use' && item.giftcard.is_give">
                                <view class="h-[70rpx] leading-[70rpx] text-[24rpx] font-500 text-center w-[calc(50%-1rpx)]" @click.stop="btnClick('give',item.card_id)">{{ t('giftToFriends') }}</view>
                                <view class="w-[2rpx] h-[24rpx] bg-[var(--text-color-light9)] rounded-[2rpx]"></view>
                            </block>
                            <view v-if="item.status=='to_use' || item.status=='can_use'" class="flex-1 h-[70rpx] leading-[70rpx] text-[24rpx] font-500 text-center min-w-[calc(50%-1rpx)]" @click.stop="btnClick('use',item.card_id)">{{ t('canUse') }}</view>
                            <view v-if="item.status=='used'" class="flex-1 h-[70rpx] leading-[70rpx] text-[24rpx] font-500 text-center min-w-[calc(50%-1rpx)]" @click.stop="btnClick('use',item.card_id)">{{ t('used') }}</view>
                            <view v-if="item.status=='invalid'" class="flex-1 h-[70rpx] leading-[70rpx] text-[24rpx] font-500 text-center min-w-[calc(50%-1rpx)]" @click.stop="btnClick('use',item.card_id)">{{ t('invalid') }}</view>
                        </view>
                    </view>
                </view>
